...
- [VRF v2 Wrapper (on-chain component)](https://github.com/smartcontractkit/chainlink/blob/develop/contracts/src/v0.8/VRFV2Wrapper.sol): a wrapper for the VRF Coordinator that provides an interface for consumer contracts.
...
The VRF Wrapper provides an interface for consumer contracts, abstracting the VRF Coordinator. Contrary to the [subscription method](/docs/vrf/v2/subscription/), users do not need to create subscriptions and pre-fund them: In fact, with the ad-hoc method, users have to directly fund their consumer contracts with LINK tokens before requesting for randomness. This is is very similar to [Connect to ANY API](/docs/request-and-receive-data/).
